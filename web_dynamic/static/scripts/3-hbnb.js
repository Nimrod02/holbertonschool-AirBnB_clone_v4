...
$(document).ready(function() {
    // Perform a POST request to retrieve place data
    $.ajax({
        type: 'POST',
        ...
        success: function(data) {
            // Loop through the data and create article tags for each place
            data.forEach(place => {
                const article = document.createElement('article');
                article.innerHTML = `<h2>${place.name}</h2>`;
                article.innerHTML += `<p>Description: ${place.description}</p>`;
    ...
    function updateApiStatus() {
        ...
        $.get('http://0.0.0.0:5001/api/v1/status', function(data) { // Fix the URL
